...
- `output/` — Generated run folders with final assets (`<OUTPUT_DIR>/<date>_<title-slug>/`). **Location is configurable**: `utils/paths.js` (`outputRoot()`/`shortsRoot()`) is the single source of truth; set `OUTPUT_DIR` in `.env` (absolute or repo-relative) to move all generated content outside the repo. Shorts default to `<OUTPUT_DIR>/shorts` (override with `SHORTS_OUTPUT_DIR`). Default when unset: `<repo>/output`. NOTE: currently set to an external path (`/Users/guru/Work/halftime-replay-output`) to keep generated media out of the repo/graphify/git.
