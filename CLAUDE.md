...
- Generated run folders with final assets live under `OUTPUT_DIR` in two parallel subdirs: **`<OUTPUT_DIR>/longs/<date>_<slug>/`** (long videos) and **`<OUTPUT_DIR>/shorts/<date>_<slug>/`** (shorts). **Location is configurable**: `utils/paths.js` is the single source of truth — `baseRoot()` = `OUTPUT_DIR`, `outputRoot()` = `<base>/longs`, `shortsRoot()` = `<base>/shorts`. Set `OUTPUT_DIR` in `.env` (absolute or repo-relative) to move all generated media outside the repo. `SHORTS_OUTPUT_DIR` still overrides shorts if set. Default when `OUTPUT_DIR` unset: `<repo>/output` (→ `<repo>/output/longs` + `/shorts`). NOTE: currently external at `/Users/guru/Work/halftime-replay-output` to keep media out of the repo/graphify/git.
...
- **Recap generation:** `ScriptWriterAgent.generateMatchRecapScript(match, {format})` (long=3 sections, short=1) + `app.generateMatchVideos(match, {formats})` in index.js: builds the scoreboard overlay (cached crests via `_buildMatchOverlay`), generates AI scene images + TTS, assembles long (1920×1080, `output/`) and/or short (1080×1920, `output/shorts/`) with the scoreboard overlay. Each metered; long writes cost+meta+seo to script.json. SEO comes from `_buildMatchSeo()` (clean match-specific tags — team names, "World Cup 2026", "match recap" — NOT the generic optimizer, which emits junk long tags YouTube rejects). The Short path passes that SEO into `ShortsProducer.produce({seo})` which now prefers a caller-provided SEO. Endpoint `POST /generate-match {matchId?, formats?}` (manual). Verified live: Germany 2-1 Ivory Coast Short rendered + uploaded (video SDYQFfFK128).
...
- **Tag sanitizer:** `PublishingSchedulingAgent._sanitizeTags()` strips `<>":`, dedupes, caps total ~480 chars, and now **drops any individual tag >30 chars** (YouTube rejects long tags as "invalid video keywords" — this bit the first WC upload, whose generic-optimizer tags were 39-55 chars). Applies to every upload.
...
### Generation queue & data-driven optimization
- **Generation queue** (`utils/generation-queue.js` + `generation_queue` DB table): single-worker **FIFO**, DB-persisted (resumes on restart; re-queues jobs stuck `running` from a prior crash). `GEN_QUEUE_DELAY_MS` (default 8000) gap between jobs. Serializes heavy renders so concurrent ffmpeg/Flux don't thrash the box (that caused 15-min render timeouts → silent fallback to plain stills). Registered runners in `index.js _registerQueueRunners()`: **`short`** (`_runShortJob`), **`match_recap`** (`_runMatchJob`), **`wc-match`** (`_runWcMatchJob`, generate + auto-upload), **`long`** (`_runLongJob`, full long-video pipeline; strategy chosen at run time). `/generate-short`, `/generate-match`, the daily-shorts cron, and `daily-content-generation` all **enqueue** (return `{queued,jobId}`); manual dashboard "Generate Short" does NOT auto-upload (draft for review). `GET /queue` + Schedule-tab widget.
- **Winning-entities optimization** (`utils/performance-insights.js`): ranks football entities (players/teams) by total views of the uploaded videos they appear in, from the marker-based `getUploadedVideosReport()`. `refreshWinningEntities()` (run nightly by `daily-optimization`) persists `data/winning-entities.json` (gitignored). `loadWinningEntities()` + `topWinners()` feed a **bias** into moment selection: `football-moments-provider.getMoment()` weighted-picks a curated moment matching a winning entity (spread across all winners by view-weight, **30% explore rate** to keep discovering), and `suggestMoments()` nudges the LLM (with a reserved wildcard). Wired into the daily-shorts cron loop and `/suggest-shorts`. **Verified live**: top entities Germany 3341v, Ronaldo 2553v, Messi 2270v across 71 Shorts. GOTCHA: the snapshot only populates when the server's YouTube client is authenticated (`apiAvailable:true`) — a standalone node script gets 0 winners; verify via the running server's "Run now" on `daily-optimization`. Extend the entity dictionary in BOTH `performance-insights.js` and `shorts-producer.js _extractEntities` together.

### Known Bugs / Risks
- **File-serving routes are now crash-safe** (`index.js`): `/output/:folder/:file` and `/short-file/:folder/:file` guard their `res.sendFile` error callbacks with `!res.headersSent` (a client abort mid-stream fired the callback after headers were sent → `ERR_HTTP_HEADERS_SENT` → unhandled → **whole server crashed**, which it did 2026-06-29). Added a 4-arg Express error handler (last middleware) + process-level `uncaughtException`/`unhandledRejection` guards in `start()` that log and keep the long-running server alive. There is NO auto-restart, though — a hard crash leaves the server down until manually restarted (consider pm2/launchd).
- `Content saved with ID: undefined` — the legacy `productions` table insert returns undefined ID (long-video pipeline only; partially broken, stale paths). **Superseded for listing** by the `generated_content` index (below) — don't rely on `productions` for "what content exists".
- **`generated_content` index** (`database/db.js`): the source of truth for WHICH videos/shorts/recaps exist — `{folder, type(long|short|match_recap), title, created_at}`, PRIMARY KEY `folder` (move-proof; not an absolute path). Written via `db.upsertContent()` (idempotent) at every generation site (long pipeline, `/generate-short`, `/short-from`, match recap long+short, daily-shorts task). `db.getContent({type})` lists it. **`/outputs` and `/shorts` read their list from this table**, then enrich each row from its folder via `app._enrichFolder()` (cost/meta/files/upload status — NOT duplicated in DB, so no drift; self-healing privacy preserved). Folders missing on disk are skipped. Backfill: `scripts/backfill-content-index.js` (seeded 36: 8 long, 7 match_recap, 21 short). This is the queryable substrate for future admin analytics.
...
- **Scheduled content tasks:** `daily-content-generation` (6 AM) makes ONE **long** 1920×1080 video via the full pipeline (strategy → script → thumbnail → SEO → `processContent`). It now **enqueues a `long` job on the generation queue** (`_runLongJob` in index.js picks the strategy at run time) rather than running the pipeline inline on the cron callback — so it serializes with shorts/recaps instead of thrashing ffmpeg/Flux. Falls back to the old inline path if the queue is unavailable. `daily-shorts-generation` (7 AM) makes N **vertical Shorts** via `runDailyShortsGeneration()` (mirrors `/generate-short`: moment → `generateShortScript` → portrait images → `ShortsProducer.produce`, each metered); it **enqueues `short` jobs with `autoUpload:true`** and **auto-uploads each at `daily_shorts_privacy`** (default **public**). N = DB setting `daily_shorts_count` (default 1, capped 10), editable from the Schedule tab (`GET`/`POST /automation/shorts-count`). Daily-Shorts moment selection is **biased toward winning entities** (see Generation queue & optimization below). NOTE: per the strategy review, Shorts vastly outperform long videos on this channel.
- **`daily-optimization` (10 PM):** `runDailyOptimization()` refreshes the **winning-entities snapshot** (`utils/performance-insights.refreshWinningEntities()`, persisted to `data/winning-entities.json`, gitignored) + cleans up old temp/upload files. Its two former steps (`optimizeExistingContent`, `updateKeywordPerformance`) were **removed** — they read `analytics_reports` JOIN `publish_schedule`, a pipeline this channel never feeds (uploads write `youtube_upload.json` markers, not `publish_schedule` rows), so they were permanent no-ops.
- YouTube publishing OAuth **is now configured** (OAuth app published to Production → refresh token no longer 7-day-expires; re-auth via `node oauth-server.js` if `invalid_grant` recurs). Uploads verified live. Earlier "OAuth not configured" notes are stale.
